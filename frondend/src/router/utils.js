...
function filterChildrenTree(data) {
...
function filterNoPermissionTree(data) {
  const currentRoles =
...
function getParentPaths(value, routes, key = "path") {
  // 深度遍历查找
...
/** 处理缓存路由（添加、删除、刷新） */
function handleAliveRoute({ name }, mode) {
  switch (mode) {
    case "add":
...
function addAsyncRoutes(arrRoutes) {
  ...
  arrRoutes.forEach((v) => {
    ...
    if (v?.children && v.children.length && !v.name)
      v.name = (v.children[0].name) + "Parent";
    if (v.meta?.frameSrc) {
      v.component = IFrame;
    } else {
